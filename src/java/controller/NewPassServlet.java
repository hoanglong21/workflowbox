...
//        processRequest(request, response);
        ...
        String pass = "";
        for (int i = 0; i < 8; i++) {
            pass += String.valueOf(abc.charAt(rd.nextInt(62)));
        }
        AccountDAO adb = new AccountDAO();
        adb.newPass(acc, pass);
        Account a = adb.getAcc(acc, pass);
        if(a==null) {
            response.sendRedirect("home");
            return;
        }
        JavaMail mail = new JavaMail(a.getEmail(), "Reset password", "Hi " + a.getFullName() + "\nYour new password: " + pass);
        request.setAttribute("msgCommon", "Check your mail");
